...
<nav class="navbar navbar-grid">
  <div class="logo">Fixlat</div>
  <button class="menu-toggle" on:click={() => menuOpen = !menuOpen} aria-label="Abrir menú">
    <span class="bar"></span>
    <span class="bar"></span>
    <span class="bar"></span>
  </button>
  <ul class:open={menuOpen}>
    <li><a href="#hero" on:click|preventDefault={() => scrollToSection('hero')}>Inicio</a></li>
...
.cta-nav {
  background: linear-gradient(90deg, #1e90ff 0%, #0072c6 100%);
  color: #fff !important;
  font-weight: bold;
  border-radius: 8px;
...
.cta-nav:hover {
  background: linear-gradient(90deg, #0072c6 0%, #1e90ff 100%);
  color: #fff !important;
...
:global(body) {
  background: linear-gradient(rgba(255,255,255,0.85), rgba(255,255,255,0.85)), url('/img/fondo.jpeg');
